...
Fase 4 completata: inviti household via `codiceInvito` (join in un household esistente in alternativa alla creazione), `VehiclesModule` con CRUD veicoli, vista "Documenti auto" e filtro documenti per tipo/veicolo, gestione membri famiglia, e UI per i pagamenti embedded nei documenti (campo facoltativo in upload per le categorie che lo prevedono, card con modifica/rimozione nel dettaglio). Prossimo: da definire (rifiniture UX, eventuale hardening multi-utente/permessi).
